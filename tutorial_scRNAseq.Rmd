...
Download gene annotation using biomart
...
# If you don't have internet connection, please uncomment and execute the following command.
...
The FeatureScatter() function is typically used to visualize feature-feature relationships, but can be used for anything calculated by the object, i.e. columns in object meta.data, PC scores etc.  
...
We filter out cells that have unique gene counts over 5000 or less than 200. Note that the parameters low.thresholds and high.thresholds are used to define a 'gate'.  -Inf and Inf should be used if you don't want a lower or upper threshold.

Furthermore, we exclude cells with a percentage of mitochondrial reads higher than 20 %.
...
Our procedure in Seurat v3 is described in detail [here](https://www.biorxiv.org/content/early/2018/11/02/460147.full.pdf), and improves on previous versions by directly modeling the mean-variance relationship inherent in single-cell data, and is implemented in the FindVariableFeatures function. By default, we return 2,000 features per dataset. These will be used in downstream analysis, like PCA.
...
# Data-driven cell type classification

SingleR is an R package for cell type classification that compares the expression patterns of every cell with population-level reference data sets and calculated probabilities for the cell type classification [(Ref.)](https://www.nature.com/articles/s41590-018-0276-y)

```{r}
input <- as.data.frame(GetAssayData(object = pbmc, slot = 'data'))
...
singleR_seurat$seurat = pbmc 
singleR_seurat$meta.data$orig.ident = pbmc$orig.ident
singleR_seurat$meta.data$clusters = pbmc$RNA_snn_res.0.3
...
```{r}
singleR_seurat$meta.data$xy = Embeddings(object = pbmc, reduction = "tsne")
